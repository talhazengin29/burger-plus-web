...
const BOS_URUN = { ad: "", fiyat: "", sira: 100, kategori: "Burgerler", urunTipi: "burger", temelMiktar: "", gorsel: "", aciklama: "", ceviriler: { ad: { en: "" }, aciklama: { en: "" } }, malzemeler: "", alerjenler: "", aktif: true, populer: false, stokTakibi: false, stokAdedi: 0, onerilenUrunler: [], gramajOpsiyonu: BOS_GRAMAJ, boyutSecenekleri: [], ekstraMalzemeAyari: BOS_EKSTRA, menuYapisi: BOS_MENU };
const BOS_KATEGORI = { ad: "", gorsel: "", sira: 10, ceviriler: { ad: { en: "" } } };
const BOS_PERSONEL = { ad: "", soyad: "", rol: "Mutfak", email: "", telefon: "", saatlikUcret: "", sifre: "" };
const BOS_DUYURU = { baslik: "", mesaj: "", hedef: "/anasayfa", ceviriler: { baslik: { en: "" }, mesaj: { en: "" } } };
const BOS_KAMPANYA = { etiket: "", baslik: "", aciklama: "", buton: "Sipariş Ver", butonTipi: "primary", gorsel: "", ikon: "🎯", aktif: true, baslangicSaat: 14, bitisSaat: 17, indirimYuzde: 10, gecerliKategoriler: [], kampanyaTipi: "surekli", sira: 10, ceviriler: { etiket: { en: "" }, baslik: { en: "" }, aciklama: { en: "" }, buton: { en: "" } } };
...
export default function Admin({ onCikis }) {
  ...
  return (
    <div className="admin-shell">
        ...
        <Modal baslik={urunForm.id ? "Ürünü düzenle" : "Yeni ürün"} aciklama="Ürün bilgileri, fiyatlandırma ve porsiyon seçenekleri" sinif="admin-modal--urun" kapat={() => setUrunForm(null)}>
          <form className="admin-form urun-duzenleme-form" onSubmit={urunKaydet}>
            ...
            <Ikili>
              ...
              <Alan etiket="Ürün adı (English)"><input maxLength="120" value={urunForm.ceviriler?.ad?.en || ""} onChange={(e) => setUrunForm({ ...urunForm, ceviriler: { ...urunForm.ceviriler, ad: { ...urunForm.ceviriler?.ad, en: e.target.value } } })} placeholder="Optional — Turkish is used as fallback" /></Alan>
            ...
            <Alan etiket="Açıklama (English)"><textarea value={urunForm.ceviriler?.aciklama?.en || ""} onChange={(e) => setUrunForm({ ...urunForm, ceviriler: { ...urunForm.ceviriler, aciklama: { ...urunForm.ceviriler?.aciklama, en: e.target.value } } })} placeholder="Optional — Turkish is used as fallback" /></Alan>
      ...
      {kategoriForm && (
        <Modal baslik={kategoriForm.id ? "Kategoriyi düzenle" : "Yeni kategori"} aciklama="Kategori adı ve görseli müşteri uygulamasındaki yuvarlak menüde kullanılır." sinif="admin-modal--kategori" kapat={() => setKategoriForm(null)}>
          <form className="admin-form kategori-form" onSubmit={kategoriKaydet}>
            ...
            <Alan etiket="Kategori adı (English)"><input maxLength="60" value={kategoriForm.ceviriler?.ad?.en || ""} onChange={(e) => setKategoriForm({ ...kategoriForm, ceviriler: { ...kategoriForm.ceviriler, ad: { ...kategoriForm.ceviriler?.ad, en: e.target.value } } })} placeholder="Example: Desserts" /></Alan>
      ...
      {kampanyaForm && (
        <Modal baslik={kampanyaForm.id ? "Kampanyayı düzenle" : "Yeni kampanya"} aciklama="Kaydettiğiniz değişiklikler müşteri uygulamasına anında yansır." sinif="admin-modal--yonetim" kapat={() => setKampanyaForm(null)}>
          <form className="admin-form" onSubmit={kampanyaKaydet}>
            ...
            <Ikili><Alan etiket="Kısa etiket (English)"><input maxLength="80" value={kampanyaForm.ceviriler?.etiket?.en || ""} onChange={(e) => setKampanyaForm({ ...kampanyaForm, ceviriler: { ...kampanyaForm.ceviriler, etiket: { en: e.target.value } } })} /></Alan><Alan etiket="Kampanya başlığı (English)"><input maxLength="120" value={kampanyaForm.ceviriler?.baslik?.en || ""} onChange={(e) => setKampanyaForm({ ...kampanyaForm, ceviriler: { ...kampanyaForm.ceviriler, baslik: { en: e.target.value } } })} /></Alan></Ikili>
            <Alan etiket="Açıklama (English)"><textarea maxLength="600" value={kampanyaForm.ceviriler?.aciklama?.en || ""} onChange={(e) => setKampanyaForm({ ...kampanyaForm, ceviriler: { ...kampanyaForm.ceviriler, aciklama: { en: e.target.value } } })} /></Alan>
            ...
            <Alan etiket="Buton metni (English)"><input maxLength="60" value={kampanyaForm.ceviriler?.buton?.en || ""} onChange={(e) => setKampanyaForm({ ...kampanyaForm, ceviriler: { ...kampanyaForm.ceviriler, buton: { en: e.target.value } } })} /></Alan>
      ...
      {duyuruForm && <Modal baslik="Yeni duyuru" kapat={() => setDuyuruForm(null)}><form className="admin-form" onSubmit={duyuruKaydet}><Alan etiket="Duyuru başlığı"><input required maxLength="100" value={duyuruForm.baslik} onChange={(e) => setDuyuruForm({ ...duyuruForm, baslik: e.target.value })} placeholder="Örn. Yeni menümüz yayında" /></Alan><Alan etiket="Duyuru başlığı (English)"><input maxLength="100" value={duyuruForm.ceviriler?.baslik?.en || ""} onChange={(e) => setDuyuruForm({ ...duyuruForm, ceviriler: { ...duyuruForm.ceviriler, baslik: { en: e.target.value } } })} /></Alan><Alan etiket="Mesaj"><textarea required maxLength="600" value={duyuruForm.mesaj} onChange={(e) => setDuyuruForm({ ...duyuruForm, mesaj: e.target.value })} placeholder="Müşterilerin bildirim panelinde göreceği açıklama" /></Alan><Alan etiket="Mesaj (English)"><textarea maxLength="600" value={duyuruForm.ceviriler?.mesaj?.en || ""} onChange={(e) => setDuyuruForm({ ...duyuruForm, ceviriler: { ...duyuruForm.ceviriler, mesaj: { en: e.target.value } } })} /></Alan><Alan etiket="Tıklandığında açılacak sayfa"><select value={duyuruForm.hedef} onChange={(e) => setDuyuruForm({ ...duyuruForm, hedef: e.target.value })}><option value="/anasayfa">Ana sayfa</option><option value="/kampanyalar">Kampanyalar</option><option value="/hediyelerim">Hediyelerim</option></select></Alan><FormAlt kapat={() => setDuyuruForm(null)} /></form></Modal>}
    </div>
  );
}
