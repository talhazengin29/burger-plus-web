...
import { i18nSozlukleriniGetir, i18nSozlukleriniKaydet } from "../../lib/adminApi";
...
export default function SozlukYonetimi() {
  ...
  useEffect(() => {
    i18nSozlukleriniGetir()
      .then(({ sozlukler: db = {} }) => {
        const gelen = { tr: db.tr || {}, en: db.en || {} };
        setDbKayitlari(gelen);
        setSozlukler({ tr: { ...CEVIRILER.tr, ...gelen.tr }, en: { ...CEVIRILER.en, ...gelen.en } });
      })
      .catch((e) => setHata(e.message))
      .finally(() => setYukleniyor(false));
  }, []);

  const anahtarlar = useMemo(() => [...new Set([...Object.keys(CEVIRILER.tr), ...Object.keys(CEVIRILER.en)])].sort(), []);
  ...
  return (
    <section className="sozluk-yonetimi">
      ...
      <div className="sozluk-ozet"><span><b>{anahtarlar.length}</b> anahtar</span><span><b>{Object.keys(dbKayitlari.en || {}).length}</b> DB çevirisi</span><span className={eksikSayisi ? "uyari" : "tamam"}><b>{eksikSayisi}</b> eksik English</span></div>
