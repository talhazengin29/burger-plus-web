...
async function excelIndir(satirlar) {
  const calisma = new ExcelJS.Workbook();
  const sayfa = calisma.addWorksheet("Platform Raporu");
  sayfa.columns = [
    { header: "İşletme", key: "ad", width: 28 },
    { header: "Ciro", key: "ciro", width: 14 },
    { header: "Sipariş", key: "siparis", width: 12 },
    { header: "Ortalama Sepet", key: "ortalamaSepet", width: 16 },
    { header: "Yeni Kullanıcı", key: "yeniKullanici", width: 16 },
  ];
  sayfa.getRow(1).font = { bold: true };
  satirlar.forEach((s) => sayfa.addRow({ ad: s.ad, ciro: s.ciro, siparis: s.siparis, ortalamaSepet: s.ortalamaSepet, yeniKullanici: s.yeniKullanici }));
  const arabellek = await calisma.xlsx.writeBuffer();
  const url = URL.createObjectURL(new Blob([arabellek], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
  const link = document.createElement("a"); link.href = url; link.download = `platform-raporu-${new Date().toISOString().slice(0, 10)}.xlsx`; link.click(); URL.revokeObjectURL(url);
}
