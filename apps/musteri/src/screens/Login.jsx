...
export default function Login() {
  ...
  return (
    <div className="ekran login">
      <DilSecici className="login-dil-secici" />
      {/* Logo — yukarıdan fade */}
      <motion.div
        className="login-ust"
      ...
      >
        ...
        <p className="login-slogan">{isletmeSlug === "burger-plus" ? t("login.slogan", "Lezzet ve puanlar seni bekliyor") : metinler.slogan}</p>
      </motion.div>
      ...
      >
        <h2 className="login-baslik">{ikiFaktorToken ? t("login.securityTitle", "Güvenlik Kodu") : t("login.title", "Giriş Yap")}</h2>

        {ikiFaktorToken ? (
          <>
            <p className="login-2fa-aciklama">{t("login.codeHelp", "Authenticator uygulamandaki 6 haneli kodu gir. Telefonuna erişemiyorsan kurtarma kodlarından birini kullanabilirsin.")}</p>
            <label className="login-etiket">{t("login.code", "Doğrulama kodu")}</label>
            <input
            ...
            <button type="button" className="login-sifremi-unuttum" onClick={() => { setIkiFaktorToken(""); setIkiFaktorKodu(""); setHata(""); }}>
              {t("login.backPassword", "Şifre ekranına dön")}
            </button>
          </>
        ) : (
          <>

        <label className="login-etiket">{t("login.email", "E-posta")}</label>
        <input
          type="email"
        ...
        <label className="login-etiket">{t("login.password", "Şifre")}</label>
        <div className="sifre-alani">
          ...
          />
          <button type="button" className="sifre-goster-btn" onClick={() => setSifreGorunur((onceki) => !onceki)} aria-label={sifreGorunur ? t("login.hidePassword", "Şifreyi gizle") : t("login.showPassword", "Şifreyi göster")} title={sifreGorunur ? t("login.hidePassword", "Şifreyi gizle") : t("login.showPassword", "Şifreyi göster")}>
        ...
        <button type="button" className="login-sifremi-unuttum" onClick={() => git("/sifremi-unuttum")}>
          {t("login.forgot", "Şifremi unuttum")}
        </button>
        ...
        <label className="beni-hatirla">
          ...
          />
          <span>{t("login.remember", "Beni hatırla")}</span>
        </label>
          </>
        )}
        ...
        <button type="submit" className="login-giris-btn" disabled={yukleniyor}>
          {yukleniyor ? t("login.loading", "Doğrulanıyor...") : ikiFaktorToken ? t("login.verify", "Kodu Doğrula") : t("login.submit", "Giriş Yap")}
        </button>
      </motion.form>
      ...
      >
        {t("login.noAccount", "Hesabın yok mu?")}{" "}
      ...
      >
        {t("login.guest", "Misafir olarak devam et")}
      </motion.button>
    </div>
  );
}
